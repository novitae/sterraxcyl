...
def Export(instructions, InfoList, rowN):
...
def makeFile(instructions): #Teableau excel de base
    def fileInfos(x):
...
def main():
    ...
    logos = [cs('|', 'DeepSkyBlue5')+bold('§').cs('LightGoldenrod2', 'DeepSkyBlue5')+cs('|', 'LightGoldenrod2'), cs('|', 'Green4')+cs('X', 'White', 'Green4')+cs('|', 'Green4'), cs('|', 'LightSalmon2')+cs('o', 'White', 'DeepPink4')+cs('|', 'MediumOrchid'), cs('|', 'Black')+bold('R').cs('Black2', 'White')+cs('|', 'Black')]
    #0 python 1 excel 2 insta 3 request
    ...
    instructions = [args.username, args.target, args.path, args.delay, None, useragent, logos, args.all_infos, instaloader.Instaloader()]
    #               0              1            2          3           4     5          6      7               8
